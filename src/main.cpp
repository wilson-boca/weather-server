...
void handleRoot() {
  String  content =
    ...
    "</head>"
    "<body>"
    "<iframe width=\"450\" height=\"260\" style=\"transform:scale(0.79);-o-transform:scale(0.79);-webkit-transform:scale(0.79);-moz-transform:scale(0.79);-ms-transform:scale(0.79);transform-origin:0 0;-o-transform-origin:0 0;-webkit-transform-origin:0 0;-moz-transform-origin:0 0;-ms-transform-origin:0 0;border: 1px solid #cccccc;\" src=\"https://thingspeak.com/channels/" + channelId + "/charts/1?bgcolor=%23ffffff&color=%23d62020&dynamic=true&type=line\"></iframe>"
    "<hr>"
    "<iframe width=\"350\" height=\"260\" style=\"border: 1px solid #cccccc;\" src=\"https://thingspeak.com/channels/1020918/widgets/162802\"></iframe>"
...
void setup() {
  ...
  Serial.println();
  SPIFFS.begin();
  Serial.println("Humidity and temperature setup");
  dht.begin();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (updateInterval > 0) {
      if (millis() - lastPub > updateInterval) {
        ...
        String item = "1=" + String(getStrength(7));
               item += "&2=" + String(dht.readHumidity());
               item += "&3=" + String(dht.readTemperature());
               item += "&4=" + String(2 * counter);
